...
public class Heap2 {
	...
	public Data3 pop() {
		...
		int tempItemsInArray = this.itemsInArray - 1;

		// Used to show how data is moved during sorting

		System.out.println("Store " + this.theHeap[0] + " in root. Store " + this.theHeap[tempItemsInArray] + " in index 0");

		System.out.println(Arrays.toString(this.theHeap) + "\n");

		Data3 root = this.theHeap[0];

		this.theHeap[0] = this.theHeap[--this.itemsInArray];
	...
	public void heapTheArray(int index) {
		...
		while (index < (this.itemsInArray / 2)) {
			...
			if ((rightChild < this.itemsInArray) && (this.theHeap[leftChild].key < this.theHeap[rightChild].key)) {

				System.out.println("Put Value " + this.theHeap[rightChild] + " in largestChild");

				largestChild = rightChild;

			} else {

				System.out.println("Put Value " + this.theHeap[leftChild] + " in largestChild");
			...
			if (root.key >= this.theHeap[largestChild].key) {
				break;
			}

			System.out.println("Put Index Value " + this.theHeap[largestChild] + " in Index " + index);

			// Save the value in largest child into the top
			// index

			this.theHeap[index] = this.theHeap[largestChild];
	...
	public void heapSort() {
		...
		for (int k = this.maxSize - 1; k >= 0; k--) {
			...
			Data3 largestNode = this.pop();
			this.insert(k, largestNode);
