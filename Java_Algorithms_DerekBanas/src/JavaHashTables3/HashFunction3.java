...
public class HashFunction3 {
	...
	public String[][] elementsToAdd = { { "ace", "Very good" }, { "act", "Take action" }, { "add", "Join (something) to something else" },
			{ "age", "Grow old" }, { "ago", "Before the present" }, { "aid", "Help, assist, or support" }, { "aim", "Point or direct" },
			...
			{ "ant", "A small insect" }, { "any", "Used to refer to one or some of a thing" }, { "ape", "A large primate" },
			{ "apt", "Appropriate or suitable in the circumstances" }, { "arc", "A part of the circumference of a curve" },
			{ "are", "Unit of measure, equal to 100 square meters" }, { "ark", "The ship built by Noah" },
			{ "arm", "Two upper limbs of the human body" }, { "art", "Expression or application of human creative skill" },
			{ "ash", "Powdery residue left after the burning" }, { "ask", "Say something in order to obtain information" },
			{ "asp", "Small southern European viper" }, { "ass", "Hoofed mammal" },
			{ "ate", "To put (food) into the mouth and swallow it" }, { "atm", "Unit of pressure" },
			{ "awe", "A feeling of reverential respect" }, { "axe", "Edge tool with a heavy bladed head" },
	...
	public int stringHashFunction(String wordToHash) {
		...
		for (int i = 0; i < wordToHash.length(); i++) {
			...
			System.out.println("Hash Key Value " + hKVTemp + " * 27 + Character Code " + charCode + " % arraySize " + this.arraySize
					+ " = " + hashKeyValue);
	...
	HashFunction3(int size) {
		...
		for (int i = 0; i < this.arraySize; i++) {
			...
			this.theArray[i] = new WordList();

		}

		this.addTheArray(this.elementsToAdd);
	...
	public void insert(Word newWord) {
		...
		int hashKey = this.stringHashFunction(wordToHash);

		// Add the new word to the array and set
		// the key for the word

		this.theArray[hashKey].insert(newWord, hashKey);
	...
	public Word find(String wordToFind) {
		...
		int hashKey = this.stringHashFunction(wordToFind);

		// NEW

		Word theWord = this.theArray[hashKey].find(hashKey, wordToFind);
	...
	public void displayTheArray() {
		...
		for (int i = 0; i < this.arraySize; i++) {
			...
			this.theArray[i].displayWordList();
